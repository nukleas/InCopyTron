...
var InCopyTron = {
    Core: {
        ...
        /* Loads javascript files in the specified folder */
            ...
            scriptFolder = new Folder(scriptFolder.parent + '/' + folder_name + '/');
            file_list = scriptFolder.getFiles();
            for (i = 0; i < file_list.length; i += 1) {
            // Checks the filetype before loading
                if (file_list[i].name.split(".")[file_list[i].name.split(".").length - 1] === file_type) {
            ...
            */
            for (plugin in InCopyTron.Plugins) {
                if (InCopyTron.Plugins.hasOwnProperty(plugin)) {
                    if (InCopyTron.Plugins[plugin].hasOwnProperty('priority') && InCopyTron.Plugins[plugin].priority === 1) {
    ...
    General_Functions: {
        ...
        open_URL_in_browser: function (URL) {
            "use strict";
            // Only works in Windows right now, will add mac version eventually
            var vbscript = 'dim objShell\rset objShell = CreateObject("Shell.Application")\rstr = "' + URL + '"\robjShell.ShellExecute str, "", "", "open", 1 ';
            try {
    ...
    Metadata: {
        ...
        build_object_from_metadata: function (server_metadata) {
            "use strict";
            var metadata_object, output = {},
                i;
            if (server_metadata !== undefined) {
                for (i = 0; i < server_metadata.length; i += 1) {
                    metadata_object = InCopyTron.Metadata.determine_type_return_metadata_object(server_metadata[i], app.k4Publications[0].k4MetaDataDefs);
                    if (metadata_object) {
        ...
        determine_type_return_metadata_object: function (k4Metadata_Object, k4Definitions) {
            "use strict";
            var metadata_result, val_list_def, pulled_item, string_def, int_def, int_value;
            if (k4Metadata_Object && k4Definitions) {
                switch (k4Metadata_Object.k4MetaDataType.toString()) {
                case "K4_STRING":
                    string_def = InCopyTron.General_Functions.getByProperty(k4Definitions, "k4Id", k4Metadata_Object.k4MetaDataDefId);
                    metadata_result = {
                ...
                case "K4_VALUE_LIST":
                    if (k4Metadata_Object.k4ValueListValue !== "30467" && k4Metadata_Object.k4ValueListValue !== "0") {
                        val_list_def = InCopyTron.General_Functions.getByProperty(k4Definitions, "k4Id", k4Metadata_Object.k4MetaDataDefId);
                        pulled_item = InCopyTron.General_Functions.getByProperty(val_list_def.k4MetaDataDefValueLists, "k4Id", k4Metadata_Object.k4ValueListValue);
                        metadata_result = {
                            'name': val_list_def.k4Name,
                            'value': pulled_item.k4Name
                        };
                        return metadata_result;
                    }
                    metadata_result = {
                        'name': InCopyTron.General_Functions.getByProperty(k4Definitions, "k4Id", k4Metadata_Object.k4MetaDataDefId).k4Name,
                        'value': ""
                    };
                    return metadata_result;
                case "K4_INTEGER":
                    int_def = InCopyTron.General_Functions.getByProperty(k4Definitions, "k4Id", k4Metadata_Object.k4MetaDataDefId);
                    switch (k4Metadata_Object.k4IntValue) {
                    case 0:
                        int_value = "no";
                        break;
                    case 1:
